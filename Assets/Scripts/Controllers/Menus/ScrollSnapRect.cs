...
public class ScrollSnapRect : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler {
    ...
    public ScrollRect scrollRectUp;
    private GameObject scrollRectUpGo;
    ...
    public GameObject buyWorldButton;
    //------------------------------------------------------------------------
    void Start() {
    ...
    private void LerpToPage(int aPageIndex) {
        ...
        _currentPage = aPageIndex;
        if (worldMenusliderController && !fileController.isWorldUnlocked(_currentPage))
        {
