...
public class ScrollSnapRectUp : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
{
    ...
    void Start()
    {
        ...
        // init
        scrollRectGo = scrollRect.gameObject;
        SetPagePositions();
    ...
    void Update()
    {
        ...
        if (_lerp)
        {
            ...
            if (Vector2.SqrMagnitude(_container.anchoredPosition - _lerpTo) < 0.25f)
            {
                ...
                _lerp = false;
                scrollRectGo.transform.GetChild(0).gameObject.SetActive(true);
    ...
    {
        scrollRectGo.transform.GetChild(0).gameObject.SetActive(false);
